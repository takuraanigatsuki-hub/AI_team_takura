...
def _can_platform_settings(user: dict | None) -> bool:
    """Cursor, GitHub, .env-интеграции — только owner / admin / tech_admin."""
    if not user:
        return False
    from room.user_auth import can_manage_site
    from room.message_filter import is_privileged
    return can_manage_site(user) or is_privileged(user.get("role", ""))


def _require_platform_settings(request: Request) -> dict:
    user = _current_user(request)
    if not _can_platform_settings(user):
...
async def auth_device_start(request: Request):
    from room.desktop_auth import start_device_flow
    base = _public_base_url(request)
    return start_device_flow(base)
...
async def auth_device_approve(body: DeviceApproveBody, request: Request):
    from room.user_auth import get_user_from_token
    from room.desktop_auth import approve_device
    user = get_user_from_token(_get_session_token(request))
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    token = _get_session_token(request)
...
async def get_config(request: Request):
    """Конфигурация: полная — только для админов; остальным — без секретов."""
    import config as cfg_module
    ...
    if user and can_view_agent_learning(user):
        public.update({
...
async def m365_status(request: Request):
    user = _optional_user(request)
    from integrations.m365_client import status as m365_status_fn
    import config as cfg_module
    st = m365_status_fn() if _can_platform_settings(user) else {"configured": False}
    st["enabled"] = cfg_module.config.get("m365_enabled", True) if _can_platform_settings(user) else False
    return st


@app.post("/api/config")
async def update_config(update: ConfigUpdate, request: Request):
    """Обновить настройки — платформа только для админов."""
    import config as cfg_module
    from room.user_auth import can_view_agent_learning

    user = _current_user(request)
    platform_touched = any(
        v is not None for v in (
            update.cursor_repo_url, update.cursor_repo_ref, update.cursor_enabled,
            update.cursor_github_sync, update.cursor_cloud_mode, update.cursor_auto_create_pr,
            update.git_auto_sync, update.git_sync_interval_sec, update.github_sync_on_tasks,
            update.m365_enabled,
        )
    )
    learning_touched = any(
        v is not None for v in (
            update.learning_interval_min, update.learning_interval_max, update.persist_knowledge,
        )
    )
    if platform_touched and not _can_platform_settings(user):
        raise HTTPException(status_code=403, detail="Настройки платформы — только для администраторов")
    if learning_touched and not (can_view_agent_learning(user) or _can_platform_settings(user)):
        raise HTTPException(status_code=403, detail="Недостаточно прав")
...
async def git_sync_now(request: Request):
...
async def cursor_run(request: CursorRunRequest, http_request: Request):
    """Запуск Cursor Agent (локально или cloud)."""
    _require_platform_settings(http_request)
    import config as cfg_module
...
async def cursor_run_status(run_id: str, request: Request):
    _require_platform_settings(request)
...
async def cursor_repositories(request: Request):
...
async def cursor_github_sync(request: CursorRunRequest, http_request: Request):
