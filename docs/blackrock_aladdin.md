...
|---|---|---|
| Factor models (Barra-style) | Частично (β-к-BTC) | Полноценная регрессия на 5-7 крипто-факторов: BTC β, ETH β, размер, momentum 4w/12w, on-chain активность |
| Monte Carlo VaR | Нет — у нас historical VaR | scipy.stats — генерация N сценариев из multivariate normal |
