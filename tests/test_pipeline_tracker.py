...
@pytest.fixture
def tracker(room):
    return PipelineTracker(room)


@pytest.mark.asyncio
async def test_awaiting_approval_marks_step_done_and_finishes(tracker, room):
    await tracker.start(
...
@pytest.mark.asyncio
async def test_get_state_clears_stale_finished(tracker, room):
...
@pytest.mark.asyncio
async def test_auto_clear_broadcasts_null(tracker, room):
    await tracker.start("Задача", {"presenter": "x"}, room.agents)
    room.task_history.tasks = [
